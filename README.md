...
- Privacy-safe: only first name and billing city are ever exposed to the browser — never surnames, emails, addresses or order numbers.
- Real orders only: with no qualifying orders the plugin shows nothing rather than inventing sales.
- Lightweight vanilla-JS popup loaded in the footer, with a configurable initial delay, display time and interval.
...
3. Configure the screen corner and timing under WooCommerce → Proof.
...
Only the first name and billing city. No surnames, emails, addresses or order
numbers are ever sent to the browser.
